...
class Map{
	...
	loadMap(){
		...
		// Function to create and add a tile layer with error handling
		function addTileLayer(tileUrl) {
			// Remove previous tile layer if exists
			if (currentTileLayer) {
				self.leaflet.map.removeLayer(currentTileLayer);
			}
			...
			currentTileLayer.on('tileerror', function(error) {
				...
				// If we get too many errors, try a different tile source
				if (tileErrorCount > 3 && tileUrl === mapTiles) {
					console.log('Trying fallback tile source');
					// Try the local proxy if direct connection fails
					addTileLayer('https://b.tile.openstreetmap.org/{z}/{x}/{y}.png');
				}
			});
			
			// Add success handler to reset error count
			currentTileLayer.on('load', function() {
				tileErrorCount = 0;
			});
			
			return currentTileLayer.addTo(self.leaflet.map);
		}
		
		// Add the initial tile layer
		const tiles = addTileLayer(mapTiles);
