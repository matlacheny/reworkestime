...
class Menu{
	...
	prepare(){
		...
		d3.json('data/indicators.json', function(error, data){
	...
	init(attrs){		
		...
		if (client.getRole() == 'controller'){
			...
			if (Object.keys(self.elements).length > 0){
				const attrs = {
						"parent": client.getParent(),
						"username": client.getUsername(),
						"elements": self.elements
				}
				var msg = createJSONMessage("elements", attrs);
...
//				break;
	...
	updateDashboards(){
		...
		// select the available clients (regardless if they are connected to other controllers)
		const available = self.clients.filter(d => d.parent != myUsername);
		
		const dropdown = d3.select('div.tab-buttons').select('div#add-dash')
			.on("click", function(d) { 	
				if (available.length == 0){
					self.errorMessage(self.language == 'en' ? 'There are no dashboards available!' : 'Aucun tableau de bord disponible!')
					return;
				}
				const dropdownContent = this.childNodes[1];
		...
		// set list of available dashboards
		dropdown.selectAll('ul').remove();	
		const ddmenu = dropdown.append('ul')
			.styles({
				'float': 'right',
				'left': 'auto',
				'right': '0px'
		...
		ddmenu.selectAll('li')
			...
			.enter()
				...
				.attr('href', '#')
				.attr('tabindex', '-1')
				.text(d => (self.language == 'en' ? 'Dashboard ' : 'Tableau de Bord ') + d.username)
				.on('click', connectDashboard)
		
		function connectDashboard(d){
			ddmenu.style('display', 'none');
			client.wsSendMessage(createJSONMessage('set-parent', {'username': client.getUsername(), 
																	'dashboard': d.username, 
																	'dataset': self.dataset, 
																	'partition': self.partition,
		...
		const updateSlider = new Promise(function(fulfill, reject){
			
			// For each connected client add a slide to the menu
			clients.forEach((c, i) => {
				
				// update tabs
				let tab = d3.select('div.tab-buttons')
					.append('div')
					.classed('tab', true)
					.text((self.language == 'en' ? 'Dashboard ' : 'Tableau de Bord ') + c.username)
					.attr('value', c.username)
					.style('font-weight', (i+1) == clients.length ? 'bold' : 'normal')
					.on('click', function(){
						let myUsername = this.getAttribute('value');
						
						d3.select('div.tab-buttons').selectAll('div.tab')
							.style('font-weight', function() { return this.getAttribute('value') == myUsername ? 'bold' :  'normal';})
					 	
						// hide the slides belonging to the other dashboards
						clients.forEach(c => {
							d3.select('div#slide-'+c.username).style('display', c.username == myUsername ? 'block' : 'none')
						})
				...
				tab.append('span')
					...
					.styles({
						...
						let username = this.parentNode.innerHTML.split(' ')[1].split('<')[0];
						
						client.wsSendMessage(createJSONMessage('unset-parent', {'dashboard': username}))
						
						self.removeElements({'username': username});
					})
				
				// add new slide
				let display = d3.select("div#slide-container").append('div')
					.attr('id', 'slide-' + c.username)
					.attr('class', 'slide')
					.style('display', (i+1) == clients.length ? 'block' : 'none')
				
				// Divide the slide in four windows as is done in each dashboard
				for (var j = 1; j <= 4; j++){
					let container = display.append('div')
						.classed('container', true)
						.attr('id', 'cont_' + j)
						
					if (j % 2 != 0)
						container.style('border-right', '1px solid')
		...
		updateSlider.then(function(){
			const displays = d3.selectAll('.display');
			
			const containers = displays.selectAll('.container') // A container is a window inside the slide
				.style('display', 'block')
			
			// the menu refer to the mini-menu inside each container for choosing and customizing the views
			const menu = containers
				.append('div')
				.styles({
					'height': '100%',
					'width': '200px',
			...
			let data = self.data['indicators'].filter(d => !['symbols', 'partition', 'datasets', 'rep_trajs'].includes(d.value))
						
			const dropdown = menu.selectAll('div')
				.data(data)
				.enter()
				.append('div')
					.classed('dropdown', true)
			...
			dropdown.selectAll('ul.dropdown-menu')
				.style('width', '100%').style('position', 'relative')			
			
			// -----------------------------------------
			// add symbols on the right
			data = self.data['indicators'].filter(d => d.value == 'symbols')[0].children;
			const imgwidth = 40;
			const divwidth = data.length * 40;
			
			const symbols = containers.append('div')
				.styles({
					'height': imgwidth+'px',
					'width': divwidth+'px',
					'cursor': 'pointer',
			...
			var svg = symbols.append('svg')
				.attr("viewBox", function() {
					return "0 0 " + divwidth + " 30"
				})
			...
			svg.selectAll('image')
				...
				.attr('height', '30px')
				.attr('transform', function(d, i){ return transformString('translate', i*imgwidth, 0); })
				.classed('button symbol', true)
				.attr('id', d => d.value+'-symbol')
				.on('click', function(d){
					const node = this.parentNode.parentNode.parentNode;
					self.key = self.getKey(node.parentNode.id.split('-')[1], node.id)
					
					switch(d.value){
					case 'clear':						
						self.sendInfo('remove')
						self.updateInputs(node.firstChild)
						break
					case 'download':
						self.download()
						break
					case 'selectspace':
						...
						break
					case 'information':
						self.errorMessage(self.language == 'en' ? 'Not yet implemented!' : "Non implementé à ce jour!")
						return;
					default:
						freeze(d.value.substring(1, d.value.length))
					}
				})
		...
		//----------------------------------------------------------------------------------------
		// Methods triggered by the symbols
		
		function changeSector(node){
			...
			if (['index', 'stc', 'map'].includes(element.depiction)) { // these views do not have the option of changing the sector
				self.errorMessage(self.language == 'en' ? 'Not available for Maps, Index Plots and the Space-Time Cube.' : "Non disponible(s) pour les Cartes, les Actogrammes, et le Cube Espace-Temps!")
				return;
			}
			...
			self.backup = {'space' : element.space}
			
			element.space = 'individual';
			self.event = {'code': 'modify', 'item': 'sector', 'value': null}
			self.openSpaceSelector(element.depiction);
		}
		
		function freeze(dim, node){				
			
			const element = self.elements[self.key];
		
			if (!element || typeof element == 'undefined'){
				self.errorMessage(self.language == 'en' ? 'There is no open view to modify.' : "Il n'y a aucune vue ouverte à modifier")
				return;
			}
			
			if (element.depiction != 'chord-diagram' && dim == 'space'){
				self.errorMessage(self.language == 'en' ? 'Only available on Flows Views.' : "Uniquement disponible(s) pour le diagramme des flux.")
				return;
			}
			...
			sessionStorage.setItem('symbols', JSON.stringify(self.data['symbols']))
		}
	}
